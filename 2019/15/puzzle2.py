...
def printMap():
    ...
    for y in range(SIZE):
        ...
        print(line)
    print(f"Bot @ {pos}, path length {len(path)}, maxDist = {maxDist}")
...
def readOutput(output):
    global pos, fmap, path, maxDist
    #print(f"Read {output}")
    draw(output)
    if(output == WALL):
        pos = path.pop()
    if (output == O2 and maxDist < 0):
        fmap = initMap()
        path = []
        pos = Position(pos.x, pos.y)
        maxDist = 0
        printMap()
        input(f"Found O2 at {pos}")
    if (maxDist >= 0 and pos.distance > maxDist):
        maxDist = pos.distance
    if (maxDist >= 0):
        printMap()
        time.sleep(0.01)
